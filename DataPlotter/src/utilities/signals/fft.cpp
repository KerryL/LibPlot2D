...
//==========================================================================
// Class:			FastFourierTransform
// Function:		DetectAliasing (static)
//
// Description:		Decimates the data and re-computes the FFT, then compares
//					results with previously computed fft data.  Only capable
//					of detecting aliasing that appears in the lower half of
//					the detectable freuqency range.
//
// Input Arguments:
//		fullFFT			= const Dataset2D& previously computed FFT data at full sample rate
//		decimatedFFT	= const Dataset2D& previously computed FFT data at reduced sample rate
//
// Output Arguments:
//		None
//
// Return Value:
//		std::string warning of any detected possible aliasing, or empty string
//		for no aliasing detected
//
//==========================================================================
std::string FastFourierTransform::DetectAliasing(const Dataset2D &fullFFT, const Dataset2D &decimatedFFT)
{
	std::set<unsigned int> fullFrequencies = FindSignificantFrequencies(fullFFT);
	std::set<unsigned int> decimatedFrequencies = FindSignificantFrequencies(decimatedFFT);

	std::set<unsigned int>::iterator i(fullFrequencies.begin()), j;
	bool increment;
	while (i != fullFrequencies.end())
	{
		increment = true;
		j = decimatedFrequencies.begin();
		while (j != decimatedFrequencies.end())
		{
			if (*i == *j)
			{
				increment = false;
				fullFrequencies.erase(i++);
				decimatedFrequencies.erase(j++);
				break;
			}
			else
				j++;
	...
	std::vector<double> aliasedFrequencies = MatchAliasedFrequencies(
		fullFrequencies, fullFFT, decimatedFrequencies, decimatedFFT);

	std::string warningString;
	if (aliasedFrequencies.size() > 0)
	{
		warningString;// FIXME:  Implement
	}
...
//==========================================================================
// Class:			FastFourierTransform
// Function:		DecimateData (static)
//
// Description:		Removes every nth point as specified and returns a new dataset.
//
// Input Arguments:
//		data			= const Dataset2D& containing complete set of data
//		reductionFactor	= const unsigned int& indicating one out of every this
//						  many points should be retained
//
// Output Arguments:
//		None
//
...
//
//==========================================================================
Dataset2D FastFourierTransform::DecimateData(const Dataset2D& data,
	const unsigned int &reductionFactor)
{
	unsigned int i;
	Dataset2D decimatedData(data.GetNumberOfPoints() / reductionFactor);
	for (i = 0; i < decimatedData.GetNumberOfPoints(); i++)
	{
		decimatedData.GetXPointer()[i] = data.GetXData(i * reductionFactor);
		decimatedData.GetYPointer()[i] = data.GetYData(i * reductionFactor);
	}
...
//==========================================================================
// Class:			FastFourierTransform
// Function:		FindSignificantFrequencies (static)
//
// Description:		Identifies significant frequencies in the FFT results.
//
// Input Arguments:
//		fft	= const Dataset2D&
//
...
//
//==========================================================================
std::set<unsigned int> FastFourierTransform::FindSignificantFrequencies(const Dataset2D &fft)
{
	// Take derivative and look for zero crossings with large negative slopes
	Dataset2D fftSlope(DiscreteDerivative::ComputeTimeHistory(fft));// yes, we know we're not really dealing with time...
	fftSlope = DiscreteDerivative::ComputeTimeHistory(fftSlope);

	double slopeThreshold(0.0);
	unsigned int i;
	for (i = 0; i < fftSlope.GetNumberOfPoints(); i++)
	{
		if (fftSlope.GetYData(i) < slopeThreshold)
			slopeThreshold = fftSlope.GetYData(i);
	}

	// Use 10% of max negative second derivative as threshold (TODO:  Remove magic number!)
	slopeThreshold *= 0.1;

	std::set<unsigned int> significantIndices;
	unsigned int recentMaxIndex, j;
	double recentMax;
	for (i = 0; i < fftSlope.GetNumberOfPoints(); i++)
	{
		if (fftSlope.GetYData(i) < slopeThreshold)
		{
			// Use the original data to ensure we're picking the peak frequency (probably
			// two or three indices back from max negative slope index)
			recentMax = 0;
			recentMaxIndex = i;
			for (j = i - std::min<unsigned int>(5, i); j <= i; j++)
			{
				if (fft.GetYData(j) > recentMax && IsPeak(fft, j))
				{
					recentMax = fft.GetYData(j);
					recentMaxIndex = j;
				}
				// Somehow we need to avoid climbing a second peak -> when going in reverse direction, if we are no longer at the peak, we're done, but how to handle it going forward?
			}

			significantIndices.insert(recentMaxIndex);
		}
	}
...
//==========================================================================
// Class:			FastFourierTransform
// Function:		IsPeak (static)
//
// Description:		Returns true if the specified index is greater than or
//					equal to both of it's neighbors.
//
// Input Arguments:
//		data	= const Dataset2D&
...
//
// Return Value:
//		true if the specified index corresponds to a peak value
//
//==========================================================================
bool FastFourierTransform::IsPeak(const Dataset2D &data, const unsigned int &i)
{
	assert (i < data.GetNumberOfPoints());
	if (i > 0)
	{
		if (data.GetYData(i) < data.GetYData(i - 1))
			return false;
	}
...
//==========================================================================
// Class:			FastFourierTransform
// Function:		MatchAliasedFrequencies (static)
//
// Description:		Compares the peak frequencies specified by the sets to
//					identify aliasing within fft1.
//
// Input Arguments:
//		set1	= std::set<unsigned int>&
//		fft1	= const Dataset2D&
//		set2	= std::set<unsigned int>&
//		fft2	= const Dataset2D&
//		shiftTolerance	= const unsigned int& indicating the allowable number of bins for a frequency to shift and still be considered a match
//		amplitudeTolerance	= const double&
//
// Output Arguments:
//		None
//
...
//
//==========================================================================
std::vector<double> FastFourierTransform::MatchAliasedFrequencies(std::set<unsigned int> &set1,
		const Dataset2D &fft1, std::set<unsigned int> &set2, const Dataset2D &fft2,
		const unsigned int& shiftTolerance, const double &amplitudeTolerance)
{
	assert(fft1.GetNumberOfPoints() > 0 && fft2.GetNumberOfPoints() > 0);

	//double fft1Resolution = fft1.GetXData(1) - fft1.GetXData(0);
	double fft2Resolution = fft2.GetXData(1) - fft2.GetXData(0);
	double fft1MaxFreq = fft1.GetXData(fft1.GetNumberOfPoints() - 1);
	double fft2MaxFreq = fft2.GetXData(fft2.GetNumberOfPoints() - 1);

	unsigned int expectedN = (unsigned int)floor(fft1MaxFreq / fft2MaxFreq + 0.5);
	unsigned int expectedIndex;

	double minDeltaAmplitude, delta;
	unsigned int minDeltaAmplitudeIndex;

	std::vector<double> aliased;

	std::set<unsigned int>::reverse_iterator i(set1.rbegin());
	std::set<unsigned int>::iterator j;
	std::set<unsigned int> nearbyFrequencies;
	...
	{
		increment = true;
		if (fft1.GetXData(*i) < fft1.GetXData(fft1.GetNumberOfPoints() - 1) / expectedN)
		{
			// Aliasing more likely... but we can still match it to set 2 and estimate actual frequency
			// FIXME:  Implement this
		}
		else
		{
			expectedIndex = (unsigned int)floor((fft2MaxFreq * (double)expectedN - fft1.GetXData(*i)) / fft2Resolution + 0.5);
			nearbyFrequencies.clear();
			for (j = set2.begin(); j != set2.end(); j++)
			{
				if (abs((int)expectedIndex - (int)*j) < (int)shiftTolerance)
					nearbyFrequencies.insert(*j);
			}

			if (nearbyFrequencies.size() == 1)
			{
				if (fabs(fft2.GetYData(*nearbyFrequencies.begin()) / fft1.GetYData(*i) - 1.0) < amplitudeTolerance)
				{
					increment = false;
					++i;
					i = std::set<unsigned int>::reverse_iterator(set1.erase(i.base()));
					set2.erase(set2.find(*nearbyFrequencies.begin()));
				}
			}
			else if (nearbyFrequencies.size() > 1)
			{
				minDeltaAmplitude = 2.0 * fabs(fft1.GetYData(*i) - fft2.GetYData(*nearbyFrequencies.begin()));
				for (j = nearbyFrequencies.begin(); j != nearbyFrequencies.end(); j++)
				{
					delta = fabs(fft1.GetYData(*i) - fft2.GetYData(*j));
					if (delta < minDeltaAmplitude &&
						delta / fft1.GetYData(*i) < amplitudeTolerance)
					{
						minDeltaAmplitude = delta;
						minDeltaAmplitudeIndex = *j;
					}
					delta /= fft1.GetYData(*i);
				}

				// FIXME:  Assumes there was a match?
				increment = false;
				++i;
				i = std::set<unsigned int>::reverse_iterator(set1.erase(i.base()));
				set2.erase(set2.find(minDeltaAmplitudeIndex));
			}
		}
